...
interface CreateSuccessModalProps {
  isOpen: boolean;
  onClose: () => void;
  eventObj: CollectionEventObj;
}

function CreateSuccessModal({ isOpen, onClose, eventObj }: CreateSuccessModalProps) {
  ...
  const { imageSrc, isLoading } = useImageLoader(eventObj.imageURI);
        ...
        <div className="px-6 py-6 sm:px-8 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{eventObj.name}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-1">Max Supply</p>
                <p className="text-lg font-semibold text-gray-900">{eventObj.maxSupply.toLocaleString()}</p>
              </div>
            </div>

            {eventObj.description && (
              <div>
                <p className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-1">Description</p>
                <p className="text-gray-700 leading-relaxed">{eventObj.description}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-1">Creator</p>
                <p className="text-sm font-mono text-gray-700">{shortenAddress(eventObj.creator)}</p>
              </div>
              <div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button className="flex-1 px-6 py-3 rounded-lg bg-linear-to-r from-gray-800 to-gray-900 hover:from-gray-700 hover:to-gray-800 text-white font-semibold transition-all duration-200 flex items-center justify-center gap-2">
            <a
                href={`https://testnet.arcscan.app/address/${eventObj.contractAddress}`}
                target="_blank"
